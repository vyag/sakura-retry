...
object Conditions {
     ...
     *
     * @param amount the maximum number of retries
     */
    data class MaxRetries(val amount: Int) : Condition {

        override fun check(context: Context): Boolean {
            return context.retryCount < amount
        }
        ...
        override fun toString(context: Context): String {
            return "context.retryCount=${context.retryCount} < $amount"
        }
    }

    /**
     * The condition check if the overall attempts is less than the given number.
     *
     * @param amount the maximum number of overall attempts
     */
    data class MaxAttempts(val amount: Int) : Condition {

        override fun check(context: Context): Boolean {
            return context.retryCount + 1 < amount
        }
        ...
        override fun toString(context: Context): String {
            return "context.retryCount=${context.retryCount} + 1 < $amount"
        }
    }
     ...
     */
    data class ExceptionOf(val errors: Set<Class<out Throwable>>) : Condition {
     ...
     */
    @JvmField
    val UNRECOVERABLE_EXCEPTIONS = ExceptionOf(InterruptedException::class.java, RuntimeException::class.java, Error::class.java)
}
